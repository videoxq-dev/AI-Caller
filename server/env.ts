...
const envSchema = z.object({
  ...
  HOSTED_SMS_PROVIDER: z.enum(["telnyx", "twilio", "plivo"]).default("telnyx"),
  HOSTED_SMS_CREDITS_PER_MESSAGE: z.coerce.number().int().positive().default(1),
  HOSTED_SMS_TELNYX_API_KEY: z.string().min(1).optional(),
  HOSTED_SMS_TELNYX_WEBHOOK_PUBLIC_KEY: z.string().min(1).optional(),
  HOSTED_SMS_TWILIO_ACCOUNT_SID: z.string().min(1).optional(),
  HOSTED_SMS_TWILIO_AUTH_TOKEN: z.string().min(1).optional(),
  HOSTED_SMS_PLIVO_AUTH_ID: z.string().min(1).optional(),
  HOSTED_SMS_PLIVO_AUTH_TOKEN: z.string().min(1).optional(),
