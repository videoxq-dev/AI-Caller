...
- New Agency clients receive an immutable `workspaceCommercialOwners.agencyClient` marker within the capacity-locked provisioning transaction. That marker persists after Agency refund; a separate Unlimited second business remains independently eligible for Unlimited. Migration `0042_agency_client_classification.sql` backfills existing additional businesses using the original business creation date and Agency purchase dates as a **one-time heuristic** (not the commercial-owner row's backfill timestamp); ambiguous imported/legacy ownership must be reconciled manually, not inferred anew in hot paths or from operational OWNER counts.
