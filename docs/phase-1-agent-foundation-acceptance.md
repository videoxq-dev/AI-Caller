...
| P1-11 | Resume Mia and retry the same question; switch to DRAFT through the status API only if authorized to do so; reactivate. | ACTIVE persists and responding resumes. DRAFT suppresses automatic replies. Private AI Agent test remains distinct from live behavior. | |
