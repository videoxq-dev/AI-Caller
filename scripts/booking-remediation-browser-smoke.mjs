...
if (!process.env.DATABASE_URL || process.env.AI_CALLER_E2E_FIXTURES !== "1" ||
    process.env.AI_CALLER_BOOKING_V2 !== "enabled") {
  throw new Error("Booking browser acceptance requires guarded fixtures and explicit test rollout.");
}
...
try {
  ...
  await pool.query("INSERT INTO business_profiles (workspace_id, business_name, timezone) VALUES ($1, 'Booking Test Workspace', 'UTC')", [workspaceId]);
  ...
  await pool.query("INSERT INTO workspace_entitlements (workspace_id,key,value) VALUES ($1,'BOOKING_ENGINE_VERSION',$2::jsonb)",
    [workspaceId,JSON.stringify("v2")]);
  ...
  await send("Tomorrow at 11 AM UTC");
  ...
  const preview=selected.rows[0],expectedStart=new Date();
  expectedStart.setUTCDate(expectedStart.getUTCDate()+1);
  expectedStart.setUTCHours(11,0,0,0);
