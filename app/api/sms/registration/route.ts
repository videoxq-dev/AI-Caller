...
export async function GET(request: Request) {
  try {
    ...
    return Response.json({
      ...
      business: business && { businessName: business.businessName, website: business.websiteUrl, phone: business.phone },
...
export async function PUT(request: Request) {
  try {
    ...
    const [registration] = await db.insert(smsRegistrations).values({
      ...
      set: { draft: input, status: "DRAFT", rejectionReason: null, updatedAt: new Date() },
    }).returning();
