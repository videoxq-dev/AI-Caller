...
export default function IntegrationsPage() {
  ...
  const [connected, setConnected] = useState<Record<ProviderId, boolean>>(() => Object.fromEntries(providers.map((provider) => [provider.id, Boolean(provider.connected)])) as Record<ProviderId, boolean>);
      ...
      <section className="appWorkspace integrationsWorkspace">
        ...
        <div className={`integrationsBody ${selected ? "drawerOpen" : ""}`}>
          ...
          {(["communication", "ai", "scheduling"] as const).map((section) => {
            ...
            return (
              <section className="integrationSection" key={section}>
...
function IntegrationCard({ provider, connected, selected, onOpen }: { provider: Provider; connected: boolean; selected: boolean; onOpen: () => void }) {
  return (
    <article className={`integrationCard ${selected ? "selected" : ""}`}>
...
function AiPanel({ provider, values, update, showSecret, setShowSecret }: PanelProps) {
  ...
  return <CredentialForm title={config.title} note="Your key is stored securely and used only for your account."><SecretField id={`${config.key}-api-key`} label="API Key" value={values.apiKey || ""} onChange={(value) => update("apiKey", value)} show={showSecret} setShow={setShowSecret} placeholder={config.placeholder} /><SelectField label="Default model" value={values.model || config.models[0]} onChange={(value) => update("model", value)} options={config.models} />{provider === "openrouter" && <Field id="openrouter-site-url" label="Site URL (optional)" value={values.siteUrl || ""} onChange={(value) => update("siteUrl", value)} placeholder="https://yourdomain.com" />}<ToggleRow title="Use provider for new conversations" subtitle="Existing conversations keep their current provider." /></CredentialForm>;
}

function SchedulingPanel({ provider, values, update, showSecret, setShowSecret }: PanelProps) {
  if (provider === "google") return <CredentialForm title="Google Calendar" note="Google Calendar uses OAuth rather than an API key for normal user accounts."><button className="oauthButton" type="button">G&nbsp;&nbsp; Connect Google account</button><SelectField label="Calendar" value={values.calendar || "Primary calendar"} onChange={(value) => update("calendar", value)} options={["Primary calendar", "Sales calendar", "Appointments"]} /><ToggleRow title="Two-way sync" subtitle="Keep AI Caller and Google Calendar in sync." /></CredentialForm>;
  if (provider === "outlook") return <CredentialForm title="Microsoft Outlook" note="Connect your Microsoft account securely with OAuth."><button className="oauthButton microsoft" type="button">M&nbsp;&nbsp; Connect Microsoft account</button><SelectField label="Calendar" value={values.calendar || "Calendar"} onChange={(value) => update("calendar", value)} options={["Calendar", "Bookings", "Team calendar"]} /><ToggleRow title="Two-way sync" subtitle="Keep AI Caller and Outlook in sync." /></CredentialForm>;
  if (provider === "calendly") return <CredentialForm title="Calendly credentials" note="Use a Calendly personal access token for this MVP connection."><SecretField id="calendly-access-token" label="Personal Access Token" value={values.token || ""} onChange={(value) => update("token", value)} show={showSecret} setShow={setShowSecret} placeholder="eyJ..." /><Field id="calendly-org-uri" label="Organization URI (optional)" value={values.org || ""} onChange={(value) => update("org", value)} placeholder="https://api.calendly.com/organizations/..." /><ToggleRow title="Sync event types" subtitle="Import active Calendly event types." /></CredentialForm>;
  return <CredentialForm title="Cal.com credentials" note="Enter your Cal.com API key."><SecretField id="calcom-api-key" label="API Key" value={values.apiKey || ""} onChange={(value) => update("apiKey", value)} show={showSecret} setShow={setShowSecret} placeholder="cal_live_..." /><Field id="calcom-slug" label="Username / team slug" value={values.slug || ""} onChange={(value) => update("slug", value)} placeholder="your-team" /><ToggleRow title="Sync event types" subtitle="Import Cal.com event types and availability." /></CredentialForm>;
}
...
function CredentialForm({ title, note, children }: { title: string; note: string; children: ReactNode }) {
  return <div className="credentialsCard"><div className="credentialIntro"><strong>{title}</strong><p>{note}</p></div><div className="credentialFields">{children}</div></div>;
}
...
function Field({ id, label, value, onChange, placeholder }: { id: string; label: string; value: string; onChange: (value: string) => void; placeholder?: string }) {
  return <label className="integrationField" htmlFor={id}><span>{label}</span><input id={id} name={`integration-${id}`} autoComplete="off" data-lpignore="true" data-1p-ignore="true" value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} /></label>;
}

function SecretField({ id, label, value, onChange, show, setShow, placeholder = "••••••••••••••••" }: { id: string; label: string; value: string; onChange: (value: string) => void; show: boolean; setShow: (value: boolean) => void; placeholder?: string }) {
  return <label className="integrationField" htmlFor={id}><span>{label}</span><div className="secretInput"><input id={id} name={`integration-secret-${id}`} type={show ? "text" : "password"} autoComplete="new-password" data-lpignore="true" data-1p-ignore="true" value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} /><button type="button" onClick={() => setShow(!show)}>{show ? "Hide" : "Show"}</button></div></label>;
}

function SelectField({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: string[] }) {
  return <label className="integrationField"><span>{label}</span><select value={value} onChange={(event) => onChange(event.target.value)}>{options.map((option) => <option key={option}>{option}</option>)}</select></label>;
}
